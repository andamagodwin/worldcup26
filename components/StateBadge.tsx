...
import { liveLabel, matchState } from '@/lib/format';
  ...
  if (state === 'live') {
    return (
      <View className="flex-row items-center gap-1 rounded-full bg-live/15 px-2 py-0.5">
        ...
        <Text className="text-[11px] font-bold text-live">{liveLabel(game) || 'LIVE'}</Text>
      </View>
    );
  }
  ...
  if (state === 'finished') {
    return (
      <View className="rounded-full bg-surface2 px-2 py-0.5">
        <Text className="text-[11px] font-bold text-muted">FT</Text>
      </View>
    );
  }

  return <Text className="text-[11px] font-semibold text-faint">{time}</Text>;
}
